...
* 注意那种只包含小写字母或者大写字母的，可以通过new char[26] 来解决
...
* 环形链表找到第一个环的入口节点
    * 先快指针走2步，慢指针走1步，如果相遇，可以确定是否有环，然后再继续走一圈拿到环的长度n，然后快指针走n步，快慢指针开始走，相遇的节点就是要找的节点。
* 想交链表
    * 找到两个链表长度的差值，然后让长度长的链表先走delta step，然后再一起走，等到节点一样时，说明是重合的点。
* 注意犯的错误, 哨兵节点，ListNode dummy = new ListNode(0), dummy.next = head; ListNode node = dummy 然后接下来，node =
  node.next 等操作，node，但是不影响dummy,最后返回dummy.next 还是list得第一个节点
...
LRU Cache: 通过map + 双向链表来实现的一种数据结构。
...
* 队列经常被用来实现二叉树的广度优先搜索。
    ...
    * 如果要区分不同的层，
        * 根据现在queue的size，来遍历，就知道是否是都是这一层的数据。
        * 或者两个队列，当前层的队列和下一层的队列。遍历完当前层，再遍历下一层。

### 8. 二叉树
* 前序遍历：根节点 → 左子树 → 右子树
* 中序遍历：左子树 → 根节点 → 右子树
* 二叉树的广度优先遍历(层序遍历): 通过队列实现。 `Queue<TreeNode> queue = new LinkedList<>()`
...
* 二叉搜索树：左子节点 <= 根节点，右子节点 >= 根节点，中序遍历是解决二叉搜索树的常见思路。
    ...
    * 左右子树的高度可以相差很大，极端情况下可能退化为链表。但是平衡二叉搜索树，最多左右子树相差1
...
* 用例：在排序数组中查找一个数字，采用二分法查找，效率最高。根据middle = (left + right) / 2 与target的值比较
* 时间复杂度O(logN)，所以如果一旦涉及到O(logN)的话，就要用二分查找。
...
可以解决集合的排列，组合等问题，通常回溯法可以通过递归实现。

特点是：解决这个问题存在多个解，题目要求往往列出所有的解。
有些难度，这次没能理解。

比较难以理解
https://programmercarl.com/回溯算法理论基础.html#理论基础

回溯的本质是穷举，所以回溯法并不高效。回溯法解决的问题：

* 组合问题：N个数里面按一定规则找出k个数的集合
* 切割问题：一个字符串按一定规则有几种切割方式
* 子集问题：一个N个子树的即合理有多少符合条件的子集
* 排列问题：全排列

组合不管元素顺序，排列要管元素顺序。

回溯法解决的问题都可以抽象成树形结构。解决回溯法的三部曲：

* 回溯函数模板返回值以及参数， void backtracking()，一般返回值是空
* 回溯函数终止条件
* 回溯搜索的遍历过程

for循环是横向遍历，遍历集合区间。backtracking 递归就是纵向遍历。
...
先遍历物品，再遍历背包 是组合数; 先遍历背包，再遍历物品是排列数。

组合不强调元素之间的顺序，排列强调元素之间的顺序。

装满背包问题，很多都是 **dp[j] += dp[j - coins[i]]**;
...
动态规划适合于: 某一问题有很多重叠子问题，每一个状态都是由上一个状态推导出来。
...
* 房屋偷盗 问题
* 在一个二维数组中，不同路径的问题。
* 股票类问题，dp[i][0] 是持有股票所得的最多现金, dp[i][1] 是不持有股票的最多现金
* 最长子序列的问题
